...
export default function WifiManagementPage() {
    ...
    async function onSubmit(data: WifiFormValues) {
        if (isReviewer) {
            toast({
                ...
                description: "Saving WiFi settings is disabled in reviewer mode.",
            });
            return;
        }
        setIsSaving(true);
    ...
    const handleDelete = async (id: string) => {
        if (isReviewer) {
            toast({
                title: "Action Disabled for Reviewer",
                description: "Deleting WiFi networks is disabled in reviewer mode.",
            });
            return;
        }
        setIsSaving(true);
    ...
    const handleEditClick = (config: WifiConfig) => {
        if (isReviewer) {
            toast({
                title: "Action Disabled for Reviewer",
                description: "Editing WiFi networks is disabled in reviewer mode.",
            });
            return;
        }
                            ...
                            {wifiConfigs.map(config => (
                                <div key={config.id} className="flex items-center justify-between p-3 border rounded-lg">
                                    ...
                                    <div className="flex gap-2">
                                        <Button variant="ghost" size="icon" onClick={() => handleEditClick(config)} disabled={isReviewer}>
                                            <Edit className="h-4 w-4" />
                                        </Button>
                                        <AlertDialog>
                                            <AlertDialogTrigger asChild>
                                                <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" disabled={isReviewer}>
                                            ...
                                            <AlertDialogContent>
                                                ...
                                                <AlertDialogFooter>
                                                    ...
                                                    <AlertDialogAction onClick={() => handleDelete(config.id)} className="bg-destructive hover:bg-destructive/90" disabled={isReviewer}>
                                                        Delete
                                                    </AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)}>
                    ...
                    <FormField
                        ...
                        <FormItem>
                            ...
                            <FormControl>
                            <Input placeholder="e.g., Taxshila-Guest" {...field} disabled={isSaving || isReviewer} />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                        )}
                    ...
                    <FormField
                        ...
                        <FormItem>
                            ...
                            <FormControl>
                            <Input placeholder="Enter WiFi password" {...field} disabled={isSaving || isReviewer} />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                        )}
                    ...
                    {editingConfig && (
                        <Button variant="ghost" onClick={() => setEditingConfig(null)} disabled={isSaving || isReviewer}>
                            Cancel Edit
                        </Button>
                    )}
                    </CardFooter>
