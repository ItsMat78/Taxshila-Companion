        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <CardDescription className="mt-1.5">Students are ordered by overdue status, then by due date.</CardDescription>
            </div>
            {feesDueStudents.length > 0 && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  const studentIds = feesDueStudents.map(s => s.studentId).join(',');
                  const title = 'Fee Payment Reminder';
                  const message = `Dear Member, this is a reminder that your library fee is due. Please clear your pending dues at the earliest to avoid any disruption in your library access. Thank you.`;
                  const alertType = 'warning';
                  router.push(
                    `/admin/alerts/send?audience=targeted&studentIds=${encodeURIComponent(studentIds)}&title=${encodeURIComponent(title)}&message=${encodeURIComponent(message)}&type=${encodeURIComponent(alertType)}`
                  );
                }}
                disabled={isLoading}
              >
