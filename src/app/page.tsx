...
import { Users, Briefcase, Armchair, CheckSquare, Clock, UserCheck, DollarSign } from 'lucide-react'; 
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { title: "Occupied Seats", value: 80, icon: Briefcase, description: "Currently in use", isClickable: true, action: () => setShowActiveStudentsDialog(true) },
    { title: "Available Seats", value: 20, icon: Armchair, description: "Ready for booking", isClickable: true, action: () => setShowAvailableSeatsDialog(true) },
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => 
          stat.isClickable ? (
            <Dialog key={stat.title} open={(stat.title === "Occupied Seats" && showActiveStudentsDialog) || (stat.title === "Available Seats" && showAvailableSeatsDialog)} onOpenChange={stat.title === "Occupied Seats" ? setShowActiveStudentsDialog : stat.title === "Available Seats" ? setShowAvailableSeatsDialog : undefined}>
