...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Otherwise, it's a real error and we should stop.
    if (error.code !== 'auth/user-not-found') {
      return NextResponse.json({ success: false, error: `Failed to delete authentication user: ${error.message}` }, { status: 500 });
    }
    console.log(`Authentication user with UID ${uid} was not found. Proceeding with database cleanup.`);
  }
  ...
  try {
    ...
    if (studentQuery.empty) {
      // If no document is found, it means the database is already clean.
      return NextResponse.json({ success: true, message: `Auth user deleted. No database records found for Student ID: ${studentId}.` });
    }
    ...
    // 2. Query and delete all related records in other collections
